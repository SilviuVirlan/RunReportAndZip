report 50100 "RSMUSAccount Schedule"
{
    DefaultLayout = RDLC;
    RDLCLayout = './src/Report/Layouts/RSMUSAccountSchedule.rdl';
    AdditionalSearchTerms = 'financial reporting,income statement,balance sheet';
    ApplicationArea = Basic, Suite;
    Caption = 'RSMUSFinancial Report';
    ...
    dataset
    {
        dataitem(AccScheduleName; "Acc. Schedule Name")
        {
            ...
            column(OneDepartment; OneDepartment) //sv+
            {
            }
                    ...
                    group("Dimension Filters")
                    {
                        ...
                        field(Dim1Filter; Dim1Filter)
                        {
                            ...
                            begin
                                UseHiddenFilters := false;
                                //sv+.begin
                                if isOneDepartment(Dim1Filter, _dimName) then
                                    OneDepartment := 'DEPARTMENT:' + _dimName;
                                //sv+.end
                            end;
                        }
    ...
    local procedure isOneDepartment(pDim1Filter: Text; var pDim1Name: Text[50]): Boolean
    var
        dims: Record "Dimension Value";
    begin
        dims.SetRange("Dimension Code", 'DEPARTMENT');
        dims.SetRange(Code, pDim1Filter);
        if dims.Count = 1 then begin
            dims.FindFirst();
            pDim1Name := dims.Name;
            exit(true);
        end;
